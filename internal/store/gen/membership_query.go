...
func (mq *MembershipQuery) QueryChat() *ChatQuery {
...
func (mq *MembershipQuery) WithChat(opts ...func(*ChatQuery)) *MembershipQuery {
	query := (&ChatClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(membership.FieldIsAdmin).
//		Aggregate(gen.Count()).
//		Scan(ctx, &v)
...
//
//	client.Membership.Query().
//		Select(membership.FieldIsAdmin).
//		Scan(ctx, &v)
...
func (mq *MembershipQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Membership, error) {
	...
	if query := mq.withChat; query != nil {
		if err := mq.loadChat(ctx, query, nodes, nil,
			func(n *Membership, e *Chat) { n.Edges.Chat = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MembershipQuery) loadChat(ctx context.Context, query *ChatQuery, nodes []*Membership, init func(*Membership), assign func(*Membership, *Chat)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Membership)
	for i := range nodes {
		if nodes[i].chat_memberships == nil {
			continue
		}
		fk := *nodes[i].chat_memberships
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "chat_memberships" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
