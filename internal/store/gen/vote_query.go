...
// QueryGather chains the current query on the "gather" edge.
func (vq *VoteQuery) QueryGather() *GatherQuery {
	query := (&GatherClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VoteQuery) Clone() *VoteQuery {
	...
	return &VoteQuery{
		...
		withGather: vq.withGather.Clone(),
...
// WithGather tells the query-builder to eager-load the nodes that are connected to
// the "gather" edge. The optional arguments are used to configure the query builder of the edge.
func (vq *VoteQuery) WithGather(opts ...func(*GatherQuery)) *VoteQuery {
	query := (&GatherClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (vq *VoteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Vote, error) {
	...
	if vq.withUser != nil || vq.withGather != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := vq.withGather; query != nil {
		if err := vq.loadGather(ctx, query, nodes, nil,
			func(n *Vote, e *Gather) { n.Edges.Gather = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VoteQuery) loadGather(ctx context.Context, query *GatherQuery, nodes []*Vote, init func(*Vote), assign func(*Vote, *Gather)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Vote)
	for i := range nodes {
		if nodes[i].gather_votes == nil {
			continue
		}
		fk := *nodes[i].gather_votes
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "gather_votes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
