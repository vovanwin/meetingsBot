...
func (cq *ChatQuery) QueryMemberships() *MembershipQuery {
	query := (&MembershipClient{config: cq.config}).Query()
...
// QueryGathers chains the current query on the "gathers" edge.
func (cq *ChatQuery) QueryGathers() *GatherQuery {
	query := (&GatherClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *ChatQuery) Clone() *ChatQuery {
	...
	return &ChatQuery{
		...
		withMemberships: cq.withMemberships.Clone(),
		withGathers:     cq.withGathers.Clone(),
...
func (cq *ChatQuery) WithMemberships(opts ...func(*MembershipQuery)) *ChatQuery {
	query := (&MembershipClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withMemberships = query
	return cq
}

// WithGathers tells the query-builder to eager-load the nodes that are connected to
// the "gathers" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *ChatQuery) WithGathers(opts ...func(*GatherQuery)) *ChatQuery {
	query := (&GatherClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withGathers = query
	return cq
}
...
func (cq *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := cq.withMemberships; query != nil {
		if err := cq.loadMemberships(ctx, query, nodes,
			func(n *Chat) { n.Edges.Memberships = []*Membership{} },
			func(n *Chat, e *Membership) { n.Edges.Memberships = append(n.Edges.Memberships, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withGathers; query != nil {
		if err := cq.loadGathers(ctx, query, nodes,
			func(n *Chat) { n.Edges.Gathers = []*Gather{} },
			func(n *Chat, e *Gather) { n.Edges.Gathers = append(n.Edges.Gathers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChatQuery) loadMemberships(ctx context.Context, query *MembershipQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *Membership)) error {
	...
	query.Where(predicate.Membership(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chat.MembershipsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.chat_memberships
		if fk == nil {
			return fmt.Errorf(`foreign-key "chat_memberships" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "chat_memberships" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (cq *ChatQuery) loadGathers(ctx context.Context, query *GatherQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *Gather)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Chat)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Gather(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chat.GathersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.chat_gathers
		if fk == nil {
			return fmt.Errorf(`foreign-key "chat_gathers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "chat_gathers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
